...
Etat au 24 septembre 2026. Ce document liste ce qui n'est pas encore fait, par lot, d'après la roadmap du README et les échanges de développement. Il ne liste pas ce qui est déjà terminé.

Fait depuis la dernière mise à jour :
- le suivi public colis/commande est branché sur la vraie API (`docs-from-api/04_SUIVI_PUBLIC_COLIS_COMMANDES_MOBILE.md`), avec la barre de recherche de la page d'accueil qui transmet réellement la référence saisie ;
- le formulaire d'enregistrement d'un colis (parcours classique : choix d'un corridor et d'un service, propriétaire, transporteur, articles saisis à la main ou envoyés en fichier, upload d'images) est branché sur la vraie API (`docs-from-api/03_ENREGISTREMENT_COLIS_MOBILE.md`), accessible depuis le menu "+" et la carte "Nouvelle expédition" de l'accueil.
...
- Parcours "Profiter de l'offre" du formulaire de colis : enregistrer un colis directement depuis une offre de groupage maritime ou aérienne ouverte (`conteneurMaritimeId` / `groupageAerienId`), corridor et mode verrouillés. Le formulaire actuel gère uniquement le parcours classique (`ligneTarifaireId`). Ce parcours dépend du point suivant.
- Module de groupage maritime/aérien dynamique avec jauge de remplissage réelle. Les écrans de détail actuels (`SeaGroupageDetailScreen`, `AirOfferDetailScreen`) affichent des chiffres fixes ; les routes réelles existent déjà (`GET /api/maritime/containers`, `GET /api/air-groupage/offers`, documentées dans `docs-from-api/03_ENREGISTREMENT_COLIS_MOBILE.md`), il reste à les brancher, puis à relier leur bouton "Participer" au parcours "Profiter de l'offre" ci-dessus.
- Liste et détail réels des colis d'un client (écran "Mes colis" actuellement un seul élément statique). Cette route n'est pas couverte par le contrat d'enregistrement de colis.
...
- Les endpoints suivants n'ont jamais été confirmés par une doc API réelle (contrairement à l'authentification, aux tarifs, à l'enregistrement de colis et au suivi public) : devis, factures, commandes (détail authentifié), liste des colis d'un client, notifications, parrainage. Leurs chemins actuels dans `api_endpoints.dart` sont des suppositions posées au tout début du projet, à vérifier avec l'équipe backend avant de les brancher.
- Le formulaire d'enregistrement de colis suppose une règle non explicitée dans la doc pour le mode `terrestre` (unité Kg par défaut, faute d'exemple concret dans le contrat) : à confirmer avec le backend si ce mode est un jour utilisé.
